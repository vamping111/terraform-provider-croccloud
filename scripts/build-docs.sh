...
upload_other_files () {
    ...
    python ../../s3cmd-2.4.0/s3cmd-2.4.0/s3cmd -c ../s3cmd.ini sync "$SOURCE_DIR" "s3://$S3_DOCS_BUCKET_NAME" --acl-public
}
...
modify_content_type () {
    ...
    find "$SOURCE_DIR" -type f -name "*.$file_type" | while read -r file; do
        python ../../s3cmd-2.4.0/s3cmd-2.4.0/s3cmd -c ../s3cmd.ini modify "s3://$S3_DOCS_BUCKET_NAME/${file#$SOURCE_DIR}" --add-header="Content-Type:$content_type"
    done
}
